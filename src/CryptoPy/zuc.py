"""
zuc.py — ZUC stream cipher (GM/T 0001-2012).

128-bit key, 128-bit IV. Generates 32-bit keystream words.
"""

from CryptoPy.core import WordArray, _32
from CryptoPy.cipher_core import StreamCipher


def _ROL32(x, n):
    n &= 0x1F
    return _32((x << n) | (x >> (32 - n)))


_S0 = bytes([
    ,
])

_S1 = bytes([
    ,
])

_D = [
    0b100010011010111, 0b010011010111100, 0b110001001101011, 0b001001101011110,
    0b101011110001001, 0b011010111100010, 0b111000100110101, 0b000100110101111,
    0b100110101111000, 0b010111100010011, 0b110101111000100, 0b001101011110001,
    0b101111000100110, 0b011110001001101, 0b111100010011010, 0b100011110101100,
]


def _BS(X):
    return (
        (_S0[(X >> 24) & 0xFF] << 24) |
        (_S1[(X >> 16) & 0xFF] << 16) |
        (_S0[(X >> 8) & 0xFF] << 8) |
        (_S1[X & 0xFF])
    )


def _L1(X):
    return _32(X ^ _ROL32(X, 2) ^ _ROL32(X, 10) ^ _ROL32(X, 18) ^ _ROL32(X, 24))


def _L2(X):
    return _32(X ^ _ROL32(X, 8) ^ _ROL32(X, 14) ^ _ROL32(X, 22) ^ _ROL32(X, 30))


class ZUC(StreamCipher):
    keySize = 128 // 32
    ivSize = 128 // 32
    blockSize = 1

    def _doReset(self):
        key = self._key
        iv = self.cfg.iv if hasattr(self.cfg, 'iv') and self.cfg.iv else WordArray.create([0] * 4)

        K = [(key.words[i] >> (24 - 8 * j)) & 0xFF for i in range(4) for j in range(4)]
        IV = [(iv.words[i] >> (24 - 8 * j)) & 0xFF for i in range(4) for j in range(4)]

        self._LFSR = [0] * 16
        self._R1 = 0
        self._R2 = 0

        for i in range(16):
            self._LFSR[i] = (K[i] << 23) | (_D[i] << 8) | IV[i]

        for _ in range(32):
            X0 = ((self._LFSR[15] >> 15) << 16) | (self._LFSR[14] & 0xFFFF)
            X1 = ((self._LFSR[11] & 0xFFFF) << 16) | (self._LFSR[9] >> 15)
            X2 = ((self._LFSR[7] & 0xFFFF) << 16) | (self._LFSR[5] >> 15)
            self._lfsr_work(self._F(X0, X1, X2) >> 1)

        # Discard first keystream word
        X0 = ((self._LFSR[15] >> 15) << 16) | (self._LFSR[14] & 0xFFFF)
        X1 = ((self._LFSR[11] & 0xFFFF) << 16) | (self._LFSR[9] >> 15)
        X2 = ((self._LFSR[7] & 0xFFFF) << 16) | (self._LFSR[5] >> 15)
        X3 = ((self._LFSR[2] & 0xFFFF) << 16) | (self._LFSR[0] >> 15)
        self._F(X0, X1, X2)  # Updates R1/R2
        self._lfsr_work()

    def _F(self, X0, X1, X2):
        W = _32((X0 ^ self._R1) + self._R2)
        W1 = _32(self._R1 + X1)
        W2 = self._R2 ^ X2
        self._R1 = _BS(_L1(((W1 & 0xFFFF) << 16) | (W2 >> 16)))
        self._R2 = _BS(_L2(((W2 & 0xFFFF) << 16) | (W1 >> 16)))
        return W

    def _lfsr_work(self, u=0):
        s = self._LFSR
        s16 = ((s[15] << 15) + (s[13] << 17) + (s[10] << 21) + (s[4] << 20) + (s[0] << 8) + s[0] + u) % 0x7FFFFFFF
        s.append(0x7FFFFFFF if s16 == 0 else s16)
        s.pop(0)

    def _doProcessBlock(self, M, offset):
        s = self._LFSR
        X0 = ((s[15] >> 15) << 16) | (s[14] & 0xFFFF)
        X1 = ((s[11] & 0xFFFF) << 16) | (s[9] >> 15)
        X2 = ((s[7] & 0xFFFF) << 16) | (s[5] >> 15)
        X3 = ((s[2] & 0xFFFF) << 16) | (s[0] >> 15)
        Z = self._F(X0, X1, X2) ^ X3
        M[offset] = _32(M[offset] ^ Z)
        self._lfsr_work()
